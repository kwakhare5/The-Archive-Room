...
// 3. Add a dividing wall partway (at column 13)
for (let r = 1; r < rows - 1; r++) {
...
layout.layoutRevision = (layout.layoutRevision || 3) + 1;

fs.writeFileSync(path, JSON.stringify(layout, null, 2));
console.log(`Room resized to ${cols}x${rows} with floors and walls. (Revision: ${layout.layoutRevision})`);
