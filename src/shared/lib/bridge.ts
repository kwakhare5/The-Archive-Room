...
export class PalaceEventBridge {
  ...
  private _handleCommand(msg: any): void {
    ...
    const normalizedPayload = msg.type === 'AGENT_COMMAND' ? msg : msg;
    const command = normalizedPayload.command;
    const agentId = normalizedPayload.agentId;
    const target = normalizedPayload.targetUid || normalizedPayload.target;
    const thought = normalizedPayload.thought || normalizedPayload.metadata?.thought;
