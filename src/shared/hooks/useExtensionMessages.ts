...
import { playDoneSound, playPermissionSound, setSoundEnabled } from '@/shared/lib/engine/notificationSound';
import type { ArchiveEngine } from '@/features/spatial/logic/ArchiveEngine';
import { setFloorSprites } from '@/shared/lib/engine/floorTiles';
import { buildDynamicCatalog } from '@/features/spatial/logic/furnitureCatalog';
import { deserializeArchive, migrateArchiveColors } from '@/features/spatial/logic/nexusSerializer';
import { setCharacterTemplates } from '@/shared/lib/engine/sprites/characterSpriteManifest';
import { extractToolName } from '@/shared/lib/engine/agentToolParser';
import type { ArchiveLayout, ToolActivity } from '@/shared/types/types';
import { setWallSprites } from '@/shared/lib/engine/wallTiles';
...
export function useExtensionMessages(
  ...
  const charactersReadyRef = useRef(false);
  const furnitureReadyRef = useRef(false);
  const wallsReadyRef = useRef(false);

  const checkAllReady = () => {
    if (
      floorReadyRef.current &&
      charactersReadyRef.current &&
      furnitureReadyRef.current &&
      wallsReadyRef.current &&
  ...
  // FAIL-SAFE: Force reveal after 6 seconds if still stuck on loading
  useEffect(() => {
    const timer = setTimeout(() => {
      if (!assetsLoaded) {
        console.warn('[Webview] Asset gate timed out. Forcing reveal.');
        setAssetsLoaded(true);
      }
    }, 6000);
  ...
  useEffect(() => {
    ...
    const handler = (e: MessageEvent) => {
      ...
      if (msg.type === 'layoutLoaded') {
        ...
        charactersReadyRef.current = true;
        checkAllReady();
        ...
        try {
          ...
          furnitureReadyRef.current = true;
          checkAllReady();
        } catch (err) {
