import type { ColorValue } from '@/shared/lib/engine/uiTypes';
import { getColorizedSprite } from '@/shared/lib/engine/colorize';
import type {
  FurnitureInstance,
  ArchiveLayout,
  PlacedFurniture,
...
import { DEFAULT_COLS, DEFAULT_ROWS, Direction, TILE_SIZE, TileType } from '@/shared/types/types';
import { getCatalogEntry, getOrientationInGroup } from './furnitureCatalog';

/** Convert flat tile array from layout into 2D grid */
export function archiveToTileMap(layout: ArchiveLayout): TileTypeVal[][] {
...
export function archiveToFurnitureInstances(furniture: PlacedFurniture[]): FurnitureInstance[] {
...
export function createDefaultArchive(): ArchiveLayout {
 ...
 * @internal */
export function serializeArchive(layout: ArchiveLayout): string {
 ...
 * @internal */
export function deserializeArchive(json: string): ArchiveLayout | null {
  try {
    const obj = JSON.parse(json);
    if (obj && obj.version === 1 && Array.isArray(obj.tiles) && Array.isArray(obj.furniture)) {
      return migrateArchive(obj as ArchiveLayout);
    }
  } catch {
    /* ignore parse errors */
  }
 ...
 */
export function migrateArchiveColors(layout: ArchiveLayout): ArchiveLayout {
  return migrateArchive(layout);
}
 ...
 */
function migrateArchive(layout: ArchiveLayout): ArchiveLayout {
