  ...
  for (let c = 1; c < cols - 1; c++) {
    ...
    if (c < 11) {
      ...
      tileColors[idx] = DEFAULT_RIGHT_ROOM_COLOR;
        ...
        // We'll fill with floor instead of wall at r=6 and r=7 for the doorway
        if (r === 6 || r === 7) {
...
// Middle Divider (with opening at r=6,7)
for (let r = 1; r < rows - 1; r++) {
  if (r === 6 || r === 7) continue; // Skip for door
...
layout.furniture = []; 
layout.layoutRevision = (layout.layoutRevision || 11) + 1;

fs.writeFileSync(path, JSON.stringify(layout, null, 2));
console.log(`Rooms balanced (10 wide each) with a DOORWAY at rows 6-7. Total size: ${cols}x${rows}. Revision: ${layout.layoutRevision}`);
