...
function getOfficeState(): OfficeState {
  if (!officeStateRef.current) {
    ...
    (window as any).getOfficeState = () => officeStateRef.current;
  }
...
function App() {
  ...
  useEffect(() => {
    if (isBrowserRuntime) {
      // Small delay to ensure useExtensionMessages has registered its listener
      setTimeout(() => {
  ...
  // Connect WebSocket bridge once assets/layout are ready
  useEffect(() => {
    if (layoutReady && officeState) {
    ...
    <div ref={containerRef} className="w-full h-full relative overflow-hidden">
      ...
      >
        <div className="text-base text-text px-10" style={{ lineHeight: 1.4 }}>
          <p className="mb-8">Your The Archive Room office now reacts in real-time:</p>
          ...
          <p className="mb-12 text-text-muted">
            This works through Claude Code Hooks, small event listeners that notify The Archive Room
