...
// We use 23 columns so we can have:
...
// Middle Divider (Exactly at index 11)
for (let r = 1; r < rows - 1; r++) {
...
layout.furniture = []; // Keep it clear
layout.layoutRevision = (layout.layoutRevision || 10) + 1;

fs.writeFileSync(path, JSON.stringify(layout, null, 2));
console.log(`Rooms balanced at 10 tiles wide each. Total size: ${cols}x${rows}. Revision: ${layout.layoutRevision}`);
